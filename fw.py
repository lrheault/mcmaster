...
class FWExtractor(sk_base.BaseEstimator, sk_base.TransformerMixin):
    ...
    def transform(self, x1,x2, **params):
        ...
        counts = self.cv.fit_transform(x1+x2).toarray()
        ...
        if isinstance(self.prior, float):
            ...
            self.priors = np.array([self.prior for i in range(vocab_size)])
        else:
        ...
        count_matrix[0, :] = np.sum(counts[:len(x1), :], axis=0)
        count_matrix[1, :] = np.sum(counts[len(x1):, :], axis=0)
        ...
        return [(index_to_term[i], count_matrix[0, i], count_matrix[1, i], z_scores[i]) for i in np.argsort(z_scores)]
